...
def load_data(ticker: str):
    ...
    cache = DataCache(
        ...
        max_age_hours=config.get('data', {}).get('cache_hours', 1)
    )
    
    df = cache.get(ticker)
    if df is None:
        fetcher = CoinFetcher([ticker])
        data = fetcher.fetch(config.get('data', {}).get('period', '5y'))
        if ticker in data:
            df = data[ticker]
...
def main():
    ...
    lookback_days = st.sidebar.slider("차트 기간 (일)", 30, 1826, 365)
    ...
    with tab1:
        ...
        for trade in filtered_trades:
            ...
            fig_home.add_trace(go.Scatter(
                ...
                showlegend=False,
                hovertemplate=f"🟢 매수: ${trade['entry_prices'][0]:,.2f}<br>{trade['entry_dates'][0].strftime('%Y-%m-%d')}<extra></extra>"
            ))
            ...
            if trade['num_buys'] > 1:
                for i in range(1, trade['num_buys']):
                    fig_home.add_trace(go.Scatter(
                        ...
                        showlegend=False,
                        hovertemplate=f"💧 물타기: ${trade['entry_prices'][i]:,.2f}<br>{trade['entry_dates'][i].strftime('%Y-%m-%d')}<extra></extra>"
                    ))
            ...
            fig_home.add_trace(go.Scatter(
                ...
                showlegend=False,
                hovertemplate=f"{sell_label}: ${trade['exit_price']:,.2f}<br>{trade['exit_date'].strftime('%Y-%m-%d')}<br>수익률: {trade['return']:+.1f}%<extra></extra>"
            ))
        ...
        for pos in current_positions:
            if pos['date'] >= signal_cutoff:
                fig_home.add_trace(go.Scatter(
                    ...
                    showlegend=False,
                    hovertemplate=f"🟠 보유중: ${pos['price']:,.2f}<br>{pos['date'].strftime('%Y-%m-%d')}<extra></extra>"
                ))
        ...
        **시그널 발생** vs **실제 매수 시그널** 구분
        ...
        if actual_buy_signals:
            ...
            buy_table = pd.DataFrame([{
                '★실제 매수일': bs['confirm_date'].strftime('%Y-%m-%d'),
                '★매수가': f"${bs['confirm_price']:,.2f}",
        ...
        if actual_sell_signals:
            ...
            sell_table = pd.DataFrame([{
                '★실제 매도일': ss['confirm_date'].strftime('%Y-%m-%d'),
                '★매도가': f"${ss['confirm_price']:,.2f}",
