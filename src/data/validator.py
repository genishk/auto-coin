...
class ValidationReport:
    ...
    def is_valid(self) -> bool:
        """데이터가 유효한지 확인"""
        return self.missing_ratio < 0.01 and self.price_valid
...
class DataValidator:
    ...
    def validate(df: pd.DataFrame, ticker: str = "UNKNOWN") -> Tuple[pd.DataFrame, ValidationReport]:
        ...
        # 3. 데이터 연속성 검증 (코인은 365일 거래라 다름)
        is_continuous = True
        if len(df_clean) > 1:
            date_diff = df_clean.index.to_series().diff()
            gaps = date_diff[date_diff > pd.Timedelta(days=3)]  # 3일 이상 갭
            if len(gaps) > 0:
                is_continuous = False
        ...
        # 5. 극단적 변동 체크 (코인은 변동성 커서 30%로)
        daily_change = df_clean['Close'].pct_change().abs()
        extreme = df_clean[daily_change > 0.3]
        if len(extreme) > 0:
            issues.append(f"일간 30%+ 변동: {len(extreme)}건 (코인에서는 정상)")
